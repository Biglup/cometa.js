...
/**
 * Registers a script stake credential and delegates to a pool.
 * @param wallet Cometa.Wallet instance to register the stake key and delegate stake.
 * @param provider Cometa.Provider instance to submit the transaction and confirm it.
 * @param script Cometa.Script instance controlling the reward account.
 * @param redeemer Cometa.PlutusData instance representing the redeemer data for the script.
 * @param rewardAddress Cometa.RewardAddress instance representing the reward address to deregister.
 * @param poolId The ID of the pool to delegate stake to.
 */
const registerAndDelegateStakeCredential = async (
  wallet: Cometa.Wallet,
  provider: Cometa.Provider,
  script: Cometa.Script,
  redeemer: Cometa.PlutusData,
  rewardAddress: Cometa.RewardAddress,
  poolId: string
  ...
  const unsignedTx = await builder
    .addScript(script)
    .registerStakeAddress({ redeemer, rewardAddress })
    .delegateStake({ poolId, redeemer, rewardAddress })
    .build();
 ...
 * @param rewardAddress Cometa.RewardAddress instance representing the reward address to deregister.
 */
const deregisterStakeCredential = async (
  wallet: Cometa.Wallet,
  provider: Cometa.Provider,
  script: Cometa.Script,
  redeemer: Cometa.PlutusData,
  ...
  monitor.startTask('Deregister stake key...');
  const builder = await wallet.createTransactionBuilder();

  const unsignedTx = await builder.addScript(script).deregisterStakeAddress({ redeemer, rewardAddress }).build();
...
/**
 * Example of registering a script stake credential, delegating it to a pool, and then deregistering it.
 */
const example = async () => {
  await Cometa.ready();

  printHeader(
    'Delegate and Withdraw Example (Script Hash)',
    'This example registers and delegates a script stake credential to a pool, and finally deregisters it.'
  );
  ...
  const rewardAddress = Cometa.RewardAddress.fromCredentials(Cometa.NetworkId.Testnet, {
    hash: scriptHash,
    type: Cometa.CredentialType.ScriptHash
  });

  await registerAndDelegateStakeCredential(wallet, provider, script, redeemer, rewardAddress, poolId);
  await deregisterStakeCredential(wallet, provider, script, redeemer, rewardAddress);
};
